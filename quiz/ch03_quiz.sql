...
SELECT name, stock
FROM products
WHERE category = '전자기기' OR stock >= 10;
...
-- 정답:
SELECT name, price + (price * 0.1) AS '10%세금 적용후 가격'
FROM products; 
...
-- 정답:
SELECT name, price - (price * 0.2) AS 'price'
FROM products
WHERE stock <= 10;
...
-- 정답:
SELECT name, stock, price - (price * 0.3) AS '30% 할인된 가격'
FROM products
...
-- 정답:
SELECT name, (price * stock) AS '재고를 모두 소진했을 때 총판매액'
FROM products;
