...
CREATE TABLE products (
  ...
  dc_rate INTEGER CHECK(dc_rate <= 50),      -- 할인율(0~50% 제한)
...
-- DATETIME DEFAULT CURRENT_TIMESTAMP: 새로운 데이터 행이 추가될 때, 
-- 해당 컬럼에 별도의 값을 지정하지 않으면 현재 날짜와 시간이 자동으로 입력됨
-- DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP: 새로운 데이터 행이 추가될 때는 물론이고,
...
-- 이 경우에는 자식 테이블레 참조하는 데이터를 모두 삭제하고 부모 테이블에서 삭젤하면 가능
