...
-- 우선 결제 유형별(GROUP BY) 평균 결제 금액 구하기 
SELECT ptype, AVG(amount)
FROM payments
GROUP BY ptype
HAVING AVG(amount) >= 40000; -- 집계 함수는 HAVING 절에서 조건을 거는 게 맞음 

-- (중요) SQL 작동 순서: FROM -> WHERE -> GROUP BY -> HAVING -> SELECT -> ORDER BY 
...
-- 따라서 집계 함수 표현식을 직접 사용하는 것이 안전하고 호환성이 높은 방법
SELECT ptype, AVG(amount) AS '평균 결제 금액'
FROM payments
GROUP BY ptype
...
-- 단일 컬럼 정렬: 결제(payments) 테이블에서 결제 금액이 높은 순서대로 조회하려면?
SELECT 
	ptype AS '결제 유형',
...
-- 다중 컬럼 정렬: 결제(payments) 테이블에서 결제 유혀은 오름차순, 결제 금액은 내림차순으로 정렬하려면? 
SELECT 
	ptype AS '결제 유형',
    amount AS '결제 금액' 
FROM payments 
ORDER BY 1, 2 DESC; -- ptype의 오름차순 정렬 후 그 안에서 amount의 내림차순 정렬(순서 중요) 

-- 실무 Tip! 
-- 오름차순인 경우 ASC 키워드는 생략 가능(정렬의 기본값) 
-- 실무에서는 오름차순인 경우에는 ASC 키워드를 대부분 생략하고 ORDER BY 만 사용 
...
-- LIMIT 절에 OFFSET 키워드를 추가해 읽어 올 데이터의 시작 지점을 조종할 수 있음 

-- 형식 
SELECT 컬럼1, 컬럼2, ... 
FROM 테이블명 
LIMIT N OFFSET M; -- N: 가져올 레코드의 수, M: 건너뛸 레코드의 개수 
-- 또는 LIMIT M, N; 
...
LIMIT 3, 3; -- 4) 처음 3개 행은 건너뛰고 3개 행만 가져옴 
...
SELECT * 
FROM products 
ORDER BY creaed_at DESC 
LIMIT 10 OFFSET 20;

-- (참고) 성능 주의사항 
-- OFFSET이 커질수록 성능이 떨어짐 (건너뛸 데이터를 계속 읽기 때문)
-- 대규모 데이터에서는 커서 기반 페이지네이션이나 키셋 페이지네이션을 고려하기도 함 
...
CREATE TABLE payments (
    ...
    order_id INT UNIQUE, -- 1:1 강제하는 방법 
    PRIMARY KEY (id),
...
-- 사용자 주문 총액 필터링하기 
-- 사용자별 주문 총액을 구하고, 그 총액이 30,000원 이상인 주문자의 주문자명과 주문총액? 
SELECT 
	u.nickname AS '주문자명', 
    SUM(p.amount) AS '주문총액' 
FROM users u 
...
SELECT 
	name AS '상품명',
	SUM(od.count * p.price) AS '누적 매출'
FROM products p
JOIN order_details od ON p.id = od.product_id
JOIN orders o ON od.order_id = o.id
			AND status = '배송 완료'
GROUP BY name
ORDER BY SUM(od.count * p.price) DESC
LIMIT 3;
