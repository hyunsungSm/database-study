...
-- 주의! DROP 명령어(특히 DROP DATABASE)는 함부로 사용하면 안됨 
...
-- 데이터 조회하기
-- SELECT 컬럼명1, 컬럼명2, ... (컬럼명 대신 *쓰면 전체 컬럼 조회)
-- FROM 테이블명 
-- WHERE 조건 (생략하면 전체 조회)
...
-- 슈비두밥 버거가 단종됐다면, 이를 위한 데이터 삭제 쿼리는?
DELETE FROM burgers
