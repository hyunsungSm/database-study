...
CREATE TABLE student_records (
    ...
    tuition_fee DECIMAL(10, 2), -- 수업료(고정 소수점 방식의 실수), 돈계산 관련은 정확하게, 전체 10자리, 소수점 이하 2자리
    PRIMARY KEY(id)
);
...
CREATE TABLE addresses (
	postal_code CHAR(5), -- 우편번호(고정 길이 문자: 5)
...
CREATE TABLE events (
	...
	created_at TIMESTAMP, 		-- 이벤트 등록 일시(YYYY-MM-DD hh:mm:ss)
...
-- LIKE '%케첩%': '케첩'을 포함한 임의의 문자 => 토마토 케첩 소스, 케첩소스, 토마토 케첩
...
-- CONCAT_WS(문자열1, 문자열2, ...): CONCAT과 비슷하지만, 첫 번째 인자로 '구분자'를 받아 각 문자열 사이에 자동으로 넣어줌
